...
**预订深链必须地理锚定（self-audit 规则）：** 任何外链（机票/酒店/交通）必须落到用户**搜索的目的地**——酒店 Booking.com 的 `ss` 必须含"城市,国家"（绝不能用裸酒店/品牌名，否则全球解析、落到错国家）；机票用 IATA 机场码。**自检工作流：** ①统一用 `app/providers/util.booking_hotel_link` + `city_label`（前端用 `src/mocks/offers.ts` 同逻辑）②回归测试 `backend/tests/test_links.py` + `__tests__/offers.test.ts`（进门禁）③线上审计路由 `GET /meta/link-audit`（逐城市校验 `ss` 含城市名，返回 `{ok,failures}`）。新增/改任何深链后必须跑通这三项。
...
- [x] **开赛日 LIVE 态**（2026-06-11 调研信号，时效最高）：Hero 倒计时归零后切「LIVE NOW + 今日赛程条」—— 2026-06-11 06:10 pm-cycle 交付：`HeroLiveStrip` 组件（LIVE 脉冲徽章 + 今日比赛横条链到 match 详情 + Full schedule 链接 + 休赛日文案；6/11–7/19 窗口外自动退场，reduce-motion 关脉冲）；门禁全绿（156 FE / 70 BE / lint / build）
- [x] **LIVE 条阶段化（进行中高亮 + FT 标记）**—— 2026-06-11 18:30 pm-cycle 交付：`src/services/matchPhase.ts`（feed `status` 优先：live/completed 为准；feed 未配则开球时间窗诚实推断——小组赛 2¼h、淘汰赛 3¼h 含加时点球，假设准点开球已注释声明）；条上每场：开球前=时间、进行中=红框「In progress」、结束=「FT」淡显；门禁全绿（164 FE / 70 BE / lint / build）。**FT 比分回显被挡**：Match 模型无比分字段，待 `SCHEDULE_FEED_URL` 激活后连模型扩展一起做
...
**下一轮建议：**
- ~~调研赛果收割~~ **已兑现（06-12 00:15）**：墨西哥 2–0 南非 + Azteca「早到 3h+轻轨
  拆段」+ Fan Fest 五城排长队 + 美国揭幕战 resale $690 < 官网 $1,120。
- 调研下轮（~06:15）：**美国揭幕战赛果**（美国 vs 巴拉圭，6/13 01:00 UTC 完赛）+ LA/SoFi
  现场与交通实测 + 韩国 vs 捷克深夜场补记；首批球迷亲历长帖（reddit）预计开始成规模。
- 产品（~06:30 到期）下一件：**多城市「追随球队」引擎（P0 护城河）**——backlog 顶项；
  起步切片建议：选队 → 该队小组赛三场跨城时间线 + 每段订票深链。
- 真实 feed 激活后跑一轮线上验证（`/matches` 对照官方赛程抽查 3 场 + LIVE 条状态正确性）；
  `SCHEDULE_FEED_URL` 仍未配置；**FT 比分回显**（feed + Match 模型加比分字段）——揭幕战
  已产生第一个真实 FT，缺口开始被用户看见，优先级上调。
- Amelia 下次按需运营钩子：①「Day 1 实战结论：早到 3h+公共交通拆段」②「smart latecomer
  收口」（美国揭幕战当天 resale 仍低于官网——今晚后换下一批捡漏场次）（daily-brief 06-12
  00:15 节）。
