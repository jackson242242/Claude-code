...
**下一轮建议：**
- **调研（~00:00 轮第一优先，已顺延一次）**：揭幕战赛果 + 首批「现场体验/交通实测/
  Fan Fest 反应」社区帖收割（18:10 到期时开球还没发生，总管主动顺延——刻意取舍非遗漏）。
- 产品（~06:30 到期）下一件：**多城市「追随球队」引擎（P0 护城河）**——LIVE 条阶段化已
  交付，backlog 顶项轮到它；起步切片建议：选队 → 该队小组赛三场跨城时间线 + 每段订票深链。
- 真实 feed 激活后跑一轮线上验证（`/matches` 对照官方赛程抽查 3 场 + LIVE 条状态正确性）；
  `SCHEDULE_FEED_URL` 仍未配置；FT 比分回显需 feed + Match 模型扩展（比分字段）一起做。
- Amelia 下次按需运营钩子升级：「零成本世界杯日」（免费 Fan Fest + Tubi 免费流 + $1.75
  接驳）+「连美国揭幕战都还能买到低于票面」（daily-brief 06-11 12:10 节）。
