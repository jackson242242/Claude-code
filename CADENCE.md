...
**下一轮建议：**
- 产品（18:10 到期）下一件：多城市「追随球队」引擎（P0 护城河），或 LIVE 条小步演进——
  调研 12:10 节信号：开球后「进行中」高亮 + 赛果 FT 比分回显是死内容变活的关键（依赖
  比赛状态数据，先评估可行性；`SCHEDULE_FEED_URL` 配置后 feed 自带 status 字段可用）。
- 调研下轮（~18:10）：**揭幕战 19:00 UTC 开球后的赛果/现场反应是收割窗口**——首批
  「现场体验/交通实测」社区帖会涌现。
- 真实 feed 激活后跑一轮线上验证（`/matches` 对照官方赛程抽查 3 场）；`SCHEDULE_FEED_URL` 仍未配置。
