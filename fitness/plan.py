...
PUSH_RULE = ("换法：主项从水平推+垂直拉各选 1 个；辅助从上斜推+水平划船+侧平举各选 1 个；"
             "三头、二头各选 1 个收尾。同一个动作连用 4 周（一个分块）再换。")
PULL_RULE = ("换法：主项从垂直拉+肩推各选 1 个；辅助从水平划船+胸/三头复合+后束各选 1 个"
             "（面拉尽量保留，它是体态动作）；手臂收尾选 2 个。同一个动作连用 4 周再换。")

SESSION_MENUS = {
    "A": ("腿部动作菜单", LEG_MENU, LEG_RULE),
    "C": ("腿部动作菜单", LEG_MENU, LEG_RULE),
    "B": ("上肢推日动作菜单", PUSH_MENU, PUSH_RULE),
    "D": ("上肢拉日动作菜单", PULL_MENU, PULL_RULE),
}
...
SESSIONS = {
    ...
    "B": dict(
        ...
        main=[
            ...
            ("坐姿高位下拉 / 引体向上", "用背带下肩胛，不是靠手臂拉"),
        ],
        acc=[
            ("上斜哑铃卧推", "30 度上斜，练上胸——这块决定 T 恤的形状"),
            ("坐姿绳索划船", "挺胸，肘贴身，末端挤背 1 秒"),
            ("哑铃侧平举", "小重量高次数，肘略高于腕，别耸肩"),
        ],
        pump=[("绳索三头下压", "肘固定在体侧"), ("哑铃二头弯举", "不摆动身体")],
        cardio="划船机 15 分钟，或动感单车间歇 8 组（40 秒冲 / 80 秒缓）",
        swap="肩前侧不适 → 杠铃卧推换哑铃卧推（更自由的轨迹）或器械推胸",
    ),
    "C": dict(
        title="下肢 B · 后链主导",
        main=[
    ...
    "D": dict(
        ...
        main=[
            ...
            ("站姿杠铃肩推 Overhead Press", "夹臀绷腹，别用腰顶；肩不适改哑铃坐姿推"),
        ],
        acc=[
            ("杠铃划船 / T 杠划船", "躯干约 45 度，拉向肚脐"),
            ("双杠臂屈伸 / 哑铃卧推", "身体略前倾偏胸，直立偏三头"),
            ("面拉 Face Pull", "拉到脸侧，外旋肩——对付含胸驼背，穿衣挺拔全靠它"),
        ],
        pump=[("锤式弯举", "中立握，练肱肌撑手臂围度"), ("绳索过顶三头伸", "充分拉伸")],
        cardio="爬楼机 15 分钟 + 雪橇推/战绳 5 组收尾（没有就快走 20 分钟）",
        swap="肩上举受限 → 肩推换斜上推（45 度）或器械肩推",
    ),
}
