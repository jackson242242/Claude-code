...
- [ ] **V3.8 徽章与 CEO 等级（老板 2026-06-13 00:37 指示）**：成就徽章体系
  （首航/首季盈利/客座率 90%+/十机机队/跨洋开拓/份额 10%/击败 AI 夺冠…，
  分铜银金三档）+ 跨局 CEO 等级（局终按表现积 XP：实习调度→区域经理→运营总监→
  航空大亨，浏览器本地档案，等级在开局画面展示）；徽章解锁即时弹出+语音祝贺。
- [ ] **V3.9 城市禀赋与开局优势（同上指示）**：cities.json 扩展禀赋字段——
  人口规模、政府税费优惠（影响总部开销/折旧）、公共交通成熟度（影响接驳需求）、
  地势类型（沿海/山地/岛屿/平原，影响天气事件易感度与跑道扩建成本）、周边航线
  网络复杂度（由邻近城市密度推导，影响竞争烈度）；**真实机场名+IATA 代码**
  （老板 00:37 补充：如 JFK 肯尼迪国际/LHR 希思罗/SIN 樟宜，中英双语，城市卡片
  与选择器展示，寓教于乐）；HQ 选择界面展示每城优劣势卡片，真正做到
  「选哪开局风格完全不同」。需契约先行（City 加字段两端同步）+平衡模拟回归。
