...
- [ ] **V2.1 世界版图**：cities.json 扩至 ~95 城（已完成数据）；后端适配 + 测试；
  前端可搜索 HQ 选择器与城市面板。
- [ ] **V2.2 机队扩充**：19 机型（含 6 经典二手：E175/737-800/A330-300/777-200ER/
  747-8i/A380，低价高油耗）；新机型 Commons 真机照片。
- [ ] **V2.3 图库扩充**：~20 个主要城市天际线照片（Commons，CC 授权+署名），
  city-images.json；HQ 选择器与城市弹层展示。
- [ ] **V2.4 音乐与角色语音**：Web Audio 程序化环境配乐（航司休息室氛围 + 现代
  游戏感，可静音）；原创虚构角色阵容（董事长/市场总监/公关顾问等，浮夸喜剧腔）
  用 SpeechSynthesis 配音播报季报与大事件，支持多语言音色。
- [ ] **V2.5 事件库扩充**：+15 条静态事件（利用新城市做地域事件），耐玩防重复。
