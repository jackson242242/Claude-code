...
<style>
  /* 2026 trend palette — Cloud Dancer canvas, Transformative Teal, Fuchsia */
  :root {{ --bg: #efede7; --card: #fdfcfa; --line: #e5e2da;
           --text: #2b2a26; --muted: #97928a;
           --accent: #0e7c7b; --fuchsia: #c7378e;
           --shadow: 0 1px 4px rgba(43, 42, 38, 0.07);
           --shadow-lift: 0 6px 18px rgba(43, 42, 38, 0.12); }}
  * {{ box-sizing: border-box; }}
  body {{ margin: 0; background: var(--bg); color: var(--text);
          font-family: system-ui, -apple-system, sans-serif; min-height: 100vh; }}
  header {{ padding: 2.2rem 1.5rem 1.2rem; border-bottom: 1px solid var(--line); background: #fff; }}
  ...
  .sound-card:hover {{ transform: translateY(-3px); box-shadow: var(--shadow-lift); }}
  ...
  .sound-card .likes {{ font-size: 0.8rem; font-weight: 700; color: var(--fuchsia); margin-top: 0.3rem; }}
  .sound-card audio {{ display: none; }}
  .playing {{ box-shadow: 0 0 0 2px var(--fuchsia), var(--shadow-lift) !important; }}
...
function drawWaveform(canvas, audioEl) {{
  ...
  bg.addColorStop(0, "hsl(178, 70%, 38%)");
  bg.addColorStop(1, "hsl(320, 55%, 52%)");
  ctx.fillStyle = bg;
