 ...
 *
 * Usage: NODE_USE_ENV_PROXY=1 node scripts/yt-competitor-scan.mjs [--queries "a,b"]  (~1000 units)
 */
...
// Query set (owner 2026-09-26 nature pivot: small towns / mountains / deserts /
// landforms are now the content spine — half the queries target that lane).
// Override with --queries "a,b,c" (100 units each).
const args = {};
for (let i = 2; i < process.argv.length; i++) if (process.argv[i].startsWith('--')) args[process.argv[i].slice(2)] = process.argv[i + 1];
const queries = args.queries ? args.queries.split(',').map((s) => s.trim()).filter(Boolean)
  : ['china travel', 'visit china', 'china hidden gems', 'china food street', 'china nature', 'china mountains',
     'china desert', 'china landscape', 'zhangjiajie', 'china village'];
const found = {};
...
const NATURE = /(mountain|desert|dune|lake|canyon|gorge|karst|terrace|glacier|waterfall|forest|grassland|plateau|valley|cave|island|village|town|zhangjiajie|guilin|yunnan|tibet|xinjiang|guizhou|sichuan|danxia|jiuzhaigou|huangshan|nature|landscape|hike|hiking)/i;
...
const natureN = top.filter((v) => NATURE.test(v.title)).length;
const lines = [`## ${new Date().toISOString().slice(0, 10)} competitor scan (top 25 China-travel Shorts published in the last 7 days, by views)`,
  `hook-shape tally: ${JSON.stringify(tally)} · nature/small-town titles in top 25: ${natureN}`,
  ...top.map(v => `- ${v.views}v ${v.likes}L (${(v.likes / v.views * 100).toFixed(1)}%) ${v.dur} | ${v.channel} | ${NATURE.test(v.title) ? '🏔 ' : ''}${v.title.slice(0, 80)} | https://youtube.com/shorts/${v.id}`),
